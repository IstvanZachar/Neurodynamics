...
#define mu_T (0.01) // mutation rate of best output (for retraining)
#define GENERATIONS (3500) // number of generations
...
int main(int argc, char** argv)
{
   ...
   int k,m,n,up,rou,rr,pro,perm[NN];
   float vect[N],best[N],pc[NN+1],pcopt,ref[NN+1],vectran[N];
   gsl_rng *r;
   ...
   for(m=0;m<NN;m++)
   {
     ...
     {     
       for(n=0;n<N;n++)
       {
         vect[n]=pattern[m][k][n];   
       }
       train_network_pali(m,vect);
     }
   }


//start with the same random pattern for all
...
// retraining RETRAINNUM number of networks
      gsl_ran_shuffle(r,perm,NN,sizeof(int));
      
      for(m=0;m<RETRAINNUM;m++)      
      {
        for(n=0;n<N;n++)
        {
	  vectran[n]=vect[n];
	  if(randd()<mu_l)
            vectran[n]=-1.0*vectran[n]; 
        }
      
        rr=perm[m];
        train_network_pali(rr,vectran);
      }
